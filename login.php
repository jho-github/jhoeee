<!doctype html>
<html lang="en">
  <head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <link rel="stylesheet" href="css/style.css">
    <title>Hello, world!</title>
  </head>
  <body class="body-login">
   <form class="form-login" action="">
     <h2 style="margin-left:20px;margin-top:10px;">Form Login</h2>
     <table>
       <tr >
         <td>
      <input class="inpt" name="email" placeholder="email">
      </td>
      
     </tr>
      <tr>
      <td>
     <input class="inpt" type="password" name="password" placeholder="password">
      </td>
      </tr>
      <tr>
        <td>
      <input style="margin-left:300px; margin-top:20px;" type="button" name="submit" value="Login">
      </td>
      </tr>
      <tr>
        <td>
          <p style="text-align:center; margin-top:20px;">belum punya akun ? <a href="#">Register</a></p>
      </td>
      </tr>
     </table>
   </form>
   <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320"><path fill="#ff5500" fill-opacity="1" d="M0,288L48,272C96,256,192,224,288,197.3C384,171,480,149,576,165.3C672,181,768,235,864,250.7C960,267,1056,245,1152,250.7C1248,256,1344,288,1392,304L1440,320L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z"></path></svg
  </body>
</html>
